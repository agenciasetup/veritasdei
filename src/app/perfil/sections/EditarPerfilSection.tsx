...
export default function EditarPerfilSection() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Limpa o input pra que selecionar o MESMO arquivo de novo dispare o
    // onChange (caso o usuário queira retentar após erro).
    if (e.target) e.target.value = ''
    if (!raw || !user) return
    setAvatarError(null)
    if (!raw.type.startsWith('image/')) {
      setAvatarError('O arquivo precisa ser uma imagem.')
      return
    }
    if (raw.size > 5 * 1024 * 1024) {
      setAvatarError('Imagem maior que 5MB. Escolhe uma menor.')
      return
    }

    setUploadingAvatar(true)
    try {
      // Sem try/catch antes, qualquer exceção em compressImage/upload
      // travava o estado em uploadingAvatar=true pra sempre — daí o
      // "looping" no spinner. O finally garante o reset.
      ...
      // Cache-bust pra browser não exibir avatar antigo após upload.
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="relative">
          ...
          {avatarError && (
            <div
              className="absolute -bottom-9 left-0 right-0 text-[10px] px-2 py-1 rounded-md text-center"
              style={{
