...
/* ──────────────────────────────────────────────────────────────────────────
   1. ESTUDAR — trilha com lições, progresso e nota aberta
   ────────────────────────────────────────────────────────────────────────── */
export function EstudarMockup({ tone = 'dark', className = '' }: MockupProps) {
  return (
    <svg viewBox="0 0 420 520" className={className} aria-hidden>
      ...
      {/* Header */}
      <text x="24" y="48" fontFamily="Cinzel, serif" fontSize="9" fill={GOLD} letterSpacing="2.5">
        TRILHA · BÍBLIA
      </text>
      <text x="24" y="76" fontFamily="Cormorant Garamond, serif" fontSize="22" fill={text(tone)} fontWeight="600">
        Carta aos Romanos
      </text>
      <text x="24" y="96" fontFamily="Poppins, sans-serif" fontSize="11" fill={muted(tone)}>
        12 lições · 3 horas
      </text>

      {/* Progress bar */}
      <rect x="24" y="116" width="372" height="6" rx="3" fill={line(tone)} />
      <rect x="24" y="116" width="186" height="6" rx="3" fill={GOLD} />
      <text x="396" y="138" textAnchor="end" fontFamily="Poppins, sans-serif" fontSize="10" fill={muted(tone)}>
        6 de 12 concluídas
      </text>

      {/* Lessons */}
      {[
        { i: 1, title: 'Saudação e tema', done: true },
        { i: 2, title: 'A justiça pela fé', done: true, current: false },
        { i: 3, title: 'Lei e graça', done: false, current: true },
      ].map((l, idx) => (
        <g key={l.i}>
          <rect
            x="24"
            ...
            height="58"
            rx="12"
            fill={l.current ? soft(tone) : tone === 'dark' ? 'rgba(255,255,255,0.025)' : '#FBF6EC'}
            stroke={l.current ? GOLD : line(tone)}
            strokeOpacity={l.current ? 0.6 : 1}
          />
          {/* status circle */}
          <circle
            cx="48"
            cy={195 + idx * 70}
            r="12"
            fill={l.done ? GOLD : 'transparent'}
            stroke={GOLD}
            strokeOpacity={l.done ? 1 : 0.55}
            strokeWidth="1.4"
          />
          {l.done && (
            <path
              d={`M42 ${195 + idx * 70} L46 ${199 + idx * 70} L54 ${191 + idx * 70}`}
              stroke={tone === 'dark' ? '#1C140C' : '#1C140C'}
              strokeWidth="2"
              fill="none"
              strokeLinecap="round"
      ...
      {/* Note pinned */}
      <g transform="rotate(-2.4 230 400)">
        <rect
          x="36"
          y="376"
          width="232"
          height="106"
          rx="10"
          fill={tone === 'dark' ? '#1F1810' : '#FFF7E5'}
          stroke={GOLD}
          strokeOpacity="0.55"
        />
        <text x="50" y="398" fontFamily="Cinzel, serif" fontSize="9" fill={GOLD} letterSpacing="1.8">
          MINHA NOTA
        </text>
        <text x="50" y="420" fontFamily="Cormorant Garamond, serif" fontSize="13" fill={text(tone)} fontStyle="italic">
          A justiça vem pela fé, mas
        </text>
        <text x="50" y="436" fontFamily="Cormorant Garamond, serif" fontSize="13" fill={text(tone)} fontStyle="italic">
          a fé sem obras é morta —
        </text>
        <text x="50" y="452" fontFamily="Cormorant Garamond, serif" fontSize="13" fill={text(tone)} fontStyle="italic">
          comparar com Tiago 2.
        </text>
        <text x="50" y="472" fontFamily="Poppins, sans-serif" fontSize="9" fill={muted(tone)}>
          hoje · lição 2
        </text>
      </g>

      {/* Trophy badge floating */}
      <g transform="translate(296 386)">
        <circle r="40" fill={soft(tone)} stroke={GOLD} strokeOpacity="0.55" />
        <path
          d="M-14 -8 Q-14 -16 -6 -16 L6 -16 Q14 -16 14 -8 Q14 4 0 8 Q-14 4 -14 -8 Z"
          fill={GOLD}
          fillOpacity="0.85"
        />
        <rect x="-3" y="6" width="6" height="10" fill={GOLD} fillOpacity="0.85" />
        <rect x="-10" y="14" width="20" height="3" rx="1.5" fill={GOLD} fillOpacity="0.85" />
        <text x="0" y="34" textAnchor="middle" fontFamily="Cinzel, serif" fontSize="8" fill={muted(tone)} letterSpacing="1.5">
          +180 XP
        </text>
      </g>
    </svg>
